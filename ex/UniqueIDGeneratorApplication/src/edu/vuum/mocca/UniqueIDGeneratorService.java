 ...
 * 
 * @brief This Service generates unique IDs within a pool of Threads. When it is
 *        created, it creates a ThreadPoolExecutor using the
...
public class UniqueIDGeneratorService extends Service {
	...
	/**
	 * A collection of unique IDs implemented internally using a persistent Java
	 * HashMap.
	 */
	private SharedPreferences uniqueIDs = null;

	/**
	 * A Messenger that encapsulates the RequestHandler used to handle request
	 * Messages sent from the UniqueIDGeneratorActivity.
	 */
	...
	public static Intent makeIntent(Context context) {
		...
		return new Intent(context, UniqueIDGeneratorService.class);
	}
			...
			mExecutor.execute(new Runnable() {
				public void run() {
					...
					try {
						...
						// UniqueIDGeneratorActivity.
						// Log.d(TAG, "replyMessenger = " +
						// replyMessenger.hashCode());
	...
	/**
	 * Factory method that returns the underlying IBinder associated with the
	 * Request Messenger.
	 */
	@Override
